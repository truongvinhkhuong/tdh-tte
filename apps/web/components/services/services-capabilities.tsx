...
import { Ruler, Weight, Users, MapPin, Wrench, Shield, Zap, Camera } from "lucide-react"
...
export function ServicesCapabilities({ lang }: ServicesCapabilitiesProps) {
    ...
    return (
        <section ref={ref} className="py-16 md:py-24 bg-gradient-to-b from-[#0f172a] to-[#1e3a75] relative overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-svn-avo-extra-bold text-white mb-4">
                        {isVi ? 'Năng lực & Cơ sở vật chất' : 'Capabilities & Facilities'}
                    </h2>
                    <p className="text-white/70 font-svn-avo-bold max-w-2xl mx-auto">
                        {isVi
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mb-16">
                    {statsData.map((stat, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={`text-center p-6 bg-white/10 backdrop-blur-sm rounded-2xl border border-white/10 hover:bg-white/15 hover:border-white/20 transition-all duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                                    }`}
                            ...
                            >
                                ...
                                <div className="text-3xl md:text-4xl font-svn-avo-extra-bold text-white mb-1">
                                    {stat.value}
                                    {stat.unit && (
                                ...
                                <div className="text-sm text-white/60 font-svn-avo-bold">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    {workshopData.map((workshop, index) => (
                        <div
                            key={workshop.id}
                            className={`group bg-white rounded-2xl overflow-hidden shadow-2xl hover:shadow-3xl transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                                }`}
                                ...
                                <ul className="space-y-2 mb-6">
                                    {(isVi ? workshop.featuresVi : workshop.featuresEn).map((feature, idx) => (
                                        <li key={idx} className="flex items-start gap-2 text-sm">
                                            <span className="w-1.5 h-1.5 bg-[#364fa1] rounded-full mt-1.5 flex-shrink-0" />
                ...
                <div className="mt-16 flex flex-wrap justify-center gap-8 md:gap-12">
                    {equipmentIcons.map((item, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={`flex items-center gap-3 text-white/70 hover:text-white transition-colors ${isVisible ? 'opacity-100' : 'opacity-0'
                                    }`}
