...
| `apps/web/components/chat/technical-chat.tsx` | — | Giao diện chat, xử lý tin nhắn, localStorage |
...
Component chính — xử lý toàn bộ logic chat.
...
- Được gửi kèm mỗi request để backend nhận diện user

**Message persistence:**
- Tối đa **20 tin nhắn** lưu trong `localStorage` với key `tte_chat_messages`
- Tin nhắn cũ bị trims khi vượt giới hạn
- Giữ nguyên khi reload trang

**Rendering:**
- Sử dụng `ReactMarkdown` + `remark-gfm` để render markdown (bảng thông số kỹ thuật, danh sách)
- Auto-scroll xuống tin nhắn mới nhất

**API Call:**

```typescript
// POST /api/rag/chat (qua Next.js middleware → NestJS Backend)
const response = await fetch('/api/rag/chat', {
  ...
  body: JSON.stringify({
    question: userInput,
    language: 'vi',           // hoặc 'en'
    sessionId: sessionId,     // UUID từ localStorage
    conversationId: undefined // optional
  })
});
```
...
### 7.2 Streaming Chat Flow (SSE)

```
[1] Frontend POST /api/rag/chat/stream
     ↓
[2] Kiểm tra cache → HIT: trả về 2 events (chunk + done)
     ↓
[3] AI Engine: POST /api/chat/stream
    → Event type "chunk": mỗi 20 ký tự với delay 20ms
    → Event type "done": metadata (confidence, sources_count)
    → Event type "error": nếu có lỗi
```

**SSE Event format:**
```
data: {"type": "chunk", "data": "Van Fisher là thiết bị"}
...
- [ ] **Conversation Memory** — Long-term memory cho returning customers
